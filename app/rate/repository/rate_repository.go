...
func (r *rateRepository) PublishRate(ctx context.Context, rate domain.RateResponse) error {
	if chn, ok := ctx.Value("channel").(string); ok {
		...
		logrus.WithField("rate", string(str)).WithField("channel", ctx.Value("channel")).Info("passing rate")
		if strings.EqualFold("ON_RATE", chn) {
			return r.wampCli.AuthClient.Publish(chn, gammazero.Dict{}, gammazero.List{string(str)}, nil)
		}
		return r.wampCli.Client.Publish(chn, gammazero.Dict{}, gammazero.List{string(str)}, nil)
	}
